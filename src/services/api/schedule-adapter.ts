...
const getEventsKey = (pageIndex: number, previousPageData: PaginatedRegularEvents) => {
...
const fetchRegularEvents = (params: EventsQueryParams): Promise<PaginatedRegularEvents> => fetcher<EventsDTO>(`${RESOURCE_URL}?${stringify(params)}`).then(mapDTOToRegularEvents);
...
export const useRegularEvents = () => (
  useSWRInfinite(getEventsKey, ([, queryParams]) => fetchRegularEvents(queryParams))
);
